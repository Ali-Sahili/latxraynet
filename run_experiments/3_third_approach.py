...
RES_DIR = "results"
res_path = "results/basic_head_classifier"
...
for name in ["basic", "specialized"]:
    print(f" ------------- {name} ------------- ")
    if name == "basic": clf_model = BasicClassifier().to(device)
        ...
        clf_model = SpecializedBasicClassifier(model_name="resnet50", state_dict=state_dict).to(device)
    else: assert "NOT SUPPORTED MODEL!!"
    train_cls(seg_model, clf_model, train_loader, val_loader, pos_weight, 
            device, model_name = name + "_head_classifier", 
