...
#-------------------------------------------------------------------------
def train_cls_with_roi(cls_model, seg_model, train_loader, val_loader, pos_weight, 
    ...
    for epoch in range(num_epochs):
        ...
        for img, mask, label in tqdm(train_loader):
            ...
            clinical = extract_geometric_features(pred_mask).to(device)

            logits = cls_model(roi_imgs, clinical).squeeze()
        ...
        with torch.no_grad():
            for img, mask, label in val_loader:
                ...
                prob = torch.sigmoid(cls_model(roi_imgs, clinical)).cpu().numpy()
